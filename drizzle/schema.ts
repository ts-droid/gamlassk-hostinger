...
/**
 * Banners table for managing site-wide announcements and notifications
 */
export const banners = mysqlTable("banners", {
  ...
  style: varchar("style", { length: 50 }).default("default").notNull(), // color scheme
  position: mysqlEnum("position", ["top", "hero", "sidebar"]).default("top").notNull(),
  active: int("active").default(1).notNull(), // 1 = active, 0 = inactive
  startDate: timestamp("startDate"),
  endDate: timestamp("endDate"),
  order: int("order").default(0).notNull(), // Display order (lower = higher priority)
  linkUrl: text("linkUrl"), // Optional link
...
/**
 * Implementation tasks table for project checklist
 */
export const implementationTasks = mysqlTable("implementation_tasks", {
  id: int("id").autoincrement().primaryKey(),
  phase: varchar("phase", { length: 100 }).notNull(), // "Fas 1", "Fas 2", etc.
  title: varchar("title", { length: 255 }).notNull(),
  description: text("description"),
