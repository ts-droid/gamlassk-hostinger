...
async function startServer() {
  ...
  // Configure Google OAuth (kept for backward compat but not used for login)
  try { configureGoogleAuth(); } catch (e) { /* Google OAuth not configured */ }
  // Legacy Manus OAuth callback (kept for backward compat)
  registerOAuthRoutes(app);
  // Password-based auth routes
  registerPasswordAuthRoutes(app);

  // Google OAuth routes (disabled - using password auth instead)
