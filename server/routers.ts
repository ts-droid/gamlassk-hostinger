...
import { implementationTasksRouter } from "./routers/implementationTasks";
import { users, news, membershipApplications, roles, galleryPhotos, events, pageContent, siteSettings, boardMembers, contentHistory, documents, paymentConfirmations, banners } from "../drizzle/schema";
import { z } from "zod";
  ...
  events: router({
    ...
    sendInvitations: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Get all active members based on selected groups
        let recipients: any[] = [];
        ...
        for (const group of input.recipientGroups) {
          if (group === 'all_active') {
            ...
            const boardMembers = await db.select().from(users).where(eq(users.role, 'admin'));
            recipients.push(...boardMembers);
          } else if (group === 'paid_current_year') {
            // Filter members who paid for current year
        ...
        const { sendEventInvitation } = await import('./_core/email');
        let sentCount = 0;
        let failedCount = 0;
  ...
  members: router({
    ...
    setPassword: manageUsersProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        await setUserPassword(input.memberId, input.password);
        return { success: true, message: 'Lösenord skapat' };
      }),

    // Reset password for member (admin only) - generates temporary password
    resetPassword: manageUsersProcedure
      .input(z.object({ memberId: z.number() }))
      .mutation(async ({ input }) => {
        const { setUserPassword } = await import('./passwordAuth');
        const { getMemberById } = await import('./db');
        ...
        await setUserPassword(input.memberId, tempPassword);
        
        return { 
          success: true, 
          tempPassword,
          message: 'Lösenord återställt. Temporärt lösenord genererat.',
        };
      }),
  }),
  ...
  documents: router({
    ...
    update: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Get current document data before updating
        const currentDoc = await db.select().from(documents).where(eq(documents.id, input.id)).limit(1);
        if (currentDoc.length === 0) {
          throw new TRPCError({ code: 'NOT_FOUND', message: 'Dokument hittades inte' });
        }
  ...
  contact: router({
    submit: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const title = `Nytt kontaktformulär: ${input.subject || 'Inget ämne'}`;
        const content = `
Från: ${input.name} (${input.email})
Ämne: ${input.subject || 'Inget ämne'}
...
`;
  ...
  banners: router({
    ...
    listAll: adminProcedure.query(async () => {
      const db = await getDb();
      if (!db) return [];
      return db.select().from(banners).orderBy(banners.order, desc(banners.createdAt));
    }),
    ...
    create: adminProcedure
      .input(z.object({
        title: z.string().min(1),
        content: z.string().min(1),
        type: z.enum(["info", "warning", "success", "event", "announcement"]),
        position: z.enum(["top", "hero", "sidebar"]),
